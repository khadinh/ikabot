...
def autoBarbariansV2(session, event, stdin_fd, predetermined_input):
    """
    Parameters
    ----------
...
def do_it(session, island, city, float_city, schematic, units_data, ship_capacity):
    sendToBot(session, "do_it started")
    attempts = {"ships": 0}
    first_loop = True
    while True:
        sendToBot(session, "Starting new attack iteration on barbarians[{}:{}].".format(island["x"], island["y"]))
        ...
        if barbarians_plan is None:
            ...
            break
        if island["barbarians"]["destroyed"] == 1:
            sendToBot(session, "Barbarians[{}:{}] are destroyed, waiting for respawn.".format(island["x"], island["y"]))
            loot(
                session,
                island,
                city,
            ...
            continue
        ships_available = waitForArrival(session)
        sendToBot(session, "amount_ships_schematic: {}, barbarians_plan: {}".format(
...
def do_attack(session, island, city, schematic, ship_capacity, float_city=None, units_data={}):
    ...
    for wave_id, wave_data in sorted(schematic["waves"].items()):
        sendToBot(session, "Starting wave {}.".format(wave_id))
        i += 1
        wave_id = int(wave_id)
        ...
        if float_city is not None:
            sendToBot(session, "Preparing attacks from float city.")
            float_city_data = []
        ...
        try:
            sendToBot(session, "Waiting for wave {} to be ready.".format(wave_id))
        ...
        sendToBot(session, "Sending wave {} attacks.".format(wave_id))
        ...
        if looting_wave:
            if len(main_city_data) > 0:
                main_city_data[0]["attack_data"]["transporter"] = min(
                    babarians_info["ships"], ships_available
                )
            elif len(float_attack_data) > 0:
                float_attack_data[0]["attack_data"]["transporter"] = min(
                    babarians_info["ships"], ships_available
                )

        if isinstance(main_city_data, list) and len(main_city_data) > 0:
            sendToBot(session, "Sending attacks from main city with data: {}".format(main_city_data))
        ...
        if isinstance(float_city_data, list) and len(float_city_data) > 0:
            sendToBot(session, "Sending attacks from float city with data: {}".format(float_city_data))
            for data in float_city_data:
                session.post(params=data["attack_data"])

        if wave_id == 1:
            sendToBot(session, "Waiting for arrival of wave {} attacks.".format(wave_id))
...
def loot(session, island, city, schematic, ship_capacity, float_city=None, units_data={}):
    sendToBot(session, "Starting looting of remaining resources in barbarians[{}:{}].".format(island["x"], island["y"]))
    ...
    while True:
        ...
        sendToBot(session, "Sent looting attack to barbarians[{}:{}], travel time: {} seconds.".format(island["x"], island["y"], travel_time))
        ...
        sendToBot(session, "Waiting for looting attack to arrive to barbarians[{}:{}].".format(island["x"], island["y"]))
